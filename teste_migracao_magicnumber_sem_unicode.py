...
def test_migration():
    ...
    for db_file, description in databases:
        if Path(db_file).exists():
            print(f"\n{'='*60}")
            print(f"TESTANDO MIGRACAO: {description} - {db_file}")
            print('='*60)
            ...
            try:
                ...
                for table_name in tables:
                    ...
                    # Para tabela game_history (já tem magic_number)
                        ...
                        print("   -> Ja possui coluna magic_number")
                        continue
                    ...
                    if needs_magic:
                        ...
                        if 'magic_number' not in column_names:
                            try:
                                ...
                                print(f"   OK Coluna magic_number adicionada a tabela {table}")
                                ...
                                if row_count > 0:
                                    ...
                                    if table.lower() == 'trades' or 'trade' in table.lower():
                                        ...
                                        print(f"   OK Magic number 777777 definido para {row_count} trades existentes")
                                    else:
                                        cursor.execute(f"UPDATE {table} SET magic_number = 888888 WHERE magic_number IS NULL")
                                        print(f"   OK Magic number 888888 definido para {row_count} registros existentes")
                                
                            except sqlite3.Error as e:
                                print(f"   ERRO ao adicionar magic_number: {e}")
                        else:
                            print(f"   OK Coluna magic_number ja existe na tabela {table}")
                    ...
                    if 'magic_number' in new_column_names:
                        print(f"   OK Migracao concluida para tabela {table}")
                    else:
                        print(f"   ERRO Migracao falhou para tabela {table}")
                
                conn.commit()
                conn.close()
                print(f"   Migracao concluida para {db_file}")
...
def create_test_trade_with_magic():
    ...
    try:
        print(f"\n{'='*60}")
        print("TESTANDO CRIACAO DE TRADE COM MAGICNUMBER")
        print('='*60)
        ...
        if result:
            print("   OK Trade de teste criado com sucesso:")
...
if __name__ == "__main__":
    print("INICIANDO TESTE DE MIGRACAO DO MAGICNUMBER")
    print("="*60)
