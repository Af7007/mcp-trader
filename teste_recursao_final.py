#!/usr/bin/env python3
"""
Teste final para verificar se a recursão infinita foi eliminada
"""
import sys
import logging
...
def test_recursion():
    """Testa se a recursão infinita foi eliminada"""
    try:
        print('Testando correção de recursão infinita...')
        from src.agents.gold_ai_agent import GoldAIAgent
        print('✅ SUCESSO: GoldAIAgent importado sem erros')
        
        # Teste básico de instanciação (sem MT5)
        ...
        if 'maximum recursion depth exceeded' in str(e).lower():
            print('❌ FALHA: Recursão ainda presente')
            print(f'Erro: {e}')
            return False
        else:
            print(f'⚠️ Erro esperado (sem MT5): {e}')
            return True
