...
# -*- coding: utf-8 -*-
"""
TESTE DE ORDEM DIRETA - GOLD (VOLUME CORRIGIDO)
================================================

OBJETIVO: Validar o ciclo completo de uma ordem, desde a abertura até o
gerenciamento do trailing stop, com VOLUME CORRIGIDO PARA 0.01.

CORREÇÃO APLICADA:
- Volume: 0.01 lote (padrão Gold, não 0.1)
- Valor do ponto: $0.001 por ponto com volume 0.01
...
class TesteOrdemDiretaCorrigido(GoldLossZeroSimple):
    """
    Agente de teste com VOLUME CORRIGIDO (0.01 lote)
    """

    def __init__(self, *args, **kwargs):
        """
        Configurações corrigidas para Gold - Volume 0.01 lote
        """
        # CORREÇÃO: Volume 0.01 lote (padrão Gold)
        kwargs.update({
            'check_interval': 2,  # Verifica a cada 2 segundos
            'volume': 0.01,      # VOLUME CORRIGIDO: 0.01 lote
        ...
        print("="*80)
        print("INICIANDO TESTE INFINITO - GOLD TRAILING STOP (VOLUME CORRIGIDO)")
        print("="*80)
        print("[OBJETIVO] Testar com volume correto (0.01 lote)")
        print("[VOLUME] 0.01 lotes (CORRIGIDO - padrão Gold)")
        print("[TRAILING] $1 ativa, protege progressivamente")
        print("[VALOR PONTO] $0.001 por ponto (com volume 0.01)")
    ...
    def _open_position(self, signal: dict):
        """
        Abre a posição de teste com VOLUME CORRIGIDO
        """
        ...
        if self.last_position_ticket:
            ...
            print(f"           Volume: {self.volume} lotes (CORRIGIDO)")
...
def main():
    ...
    """
    agent = TesteOrdemDiretaCorrigido(symbol='XAUUSDc')
    agent.run()
