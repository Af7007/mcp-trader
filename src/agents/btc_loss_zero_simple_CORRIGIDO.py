...
class BTCLossZeroSimple:
    ...
    def __init__(
        self,
        symbol: str = "BTCUSDc",
        volume: float = 0.03,  # AJUSTADO: 0.03 lotes (mais ágil)
        check_interval: int = 15,
        stop_loss_atr_multiplier: float = 1.2,  # AJUSTADO: SL = ATR × 1.2 (mais apertado)
        trailing_activation_atr_multiplier: float = 0.3,  # AJUSTADO: ATR × 0.3 (ativa cedo)
        trailing_distance_atr_multiplier: float = 0.2,  # AJUSTADO: ATR × 0.2 (mais próximo)
        use_buy: bool = True,
        use_sell: bool = True
    ):
        """
        ...
        # Valor do ponto para BTC (será calculado dinamicamente)
        ...
        print(f"Agente BTC Loss Zero - ESTRATEGIA TRAILING STOP v1.3 AGIL")
        print(f"   Symbol: {self.symbol}")
        print(f"   Volume: {self.volume} lotes (padrao 0.03, max 0.1)")
        print(f"   SL: ATR × {self.sl_atr_mult} (80-120 pts - AGIL)")
        print(f"   TP: SEM TP FIXO (trailing cuida)")
        print(f"   Trailing ativa: ATR × {self.trailing_activation_mult} (~24-30 pts)")
        print(f"   Trailing distancia: ATR × {self.trailing_distance_mult} (~16-20 pts)")
        ...
        print(f"   BTC Logger: ATIVO")
        print(f"")
        print(f"   CONFIGURACAO v1.3 AGIL:")
        print(f"   - SL: 80-120 pontos (mais apertado)")
        print(f"   - Volume: 0.03 lotes (3x mais movimento)")
        print(f"   - Trailing ativa: ~30 pts de lucro (rapido!)")
        print(f"   - Trailing distancia: ~20 pts (proximo)")
        print(f"   - Operacoes fecham em minutos, nao horas!")
        print(f"")
        print(f"   ESTRATEGIA:")
        print(f"   1. SL apertado fecha rapido se errar")
        print(f"   2. Trailing ativa com pouco lucro (~30 pts)")
        print(f"   3. Fecha lucro pequeno mas rapido")
        print(f"   4. Mais operacoes por dia")
    ...
    def _calculate_point_value(self):
        ...
        Para BTC:
        - Contract size: varia por broker (tipicamente 0.01 ou 1)
        - Point: 0.01 (para BTCUSDc)
        - trade_tick_value: valor fornecido pelo MT5
        """
        try:
            ...
            if symbol_info:
                ...
                # Fallback para BTC padrão
                self.point_value = 0.01  # Valor típico para BTC
                print(f"[BTC] Usando valor padrão: ${self.point_value:.2f} por lote")
    ...
    def _pontos_para_dinheiro(self, pontos: float) -> float:
        """
        Converte pontos para dinheiro considerando volume e point value.
        ...
        # Para Gold: pontos × volume × point_value
        # Exemplo: 24 pontos × 0.01 lote × $1.00/ponto = $0.24
        return pontos * self.volume * self.point_value


    def run(self):
        """
        Executa agente Loss Zero com monitoramento contínuo
        """
        print(f"AGENTE BTC LOSS ZERO - TRAILING ILIMITADO + MONITORAMENTO CONTINUO")
        print(f"="*60)
        ...
        try:
            ...
            # Garantir que worker é parado
            if self.position_worker and self.position_worker.is_running():
                print("Parando worker de monitoramento...")
    ...
    def _analyze_m5_trend(self, rates) -> dict:
        ...
        try:
            ...
            # === THRESHOLDS BALANCEADOS PARA BTC - MODO BALANCEADO ===
            # AJUSTADO: Reduzido para capturar mais movimentos
            MOMENTUM_BUY = 0.03   # 0.03% = ~$33 movimento em BTC $110k (era 0.04%)
            MOMENTUM_SELL = -0.03
            ...
            if self.use_buy:
                ...
                if momentum_5m > MOMENTUM_BUY * 1.5:  # 0.045%
                    confirmations += 1

                # Confirmação 3: Volatilidade OU Volume + Preço (RELAXADO: OR em vez de AND)
            ...
            if self.use_sell:
                ...
                # Confirmação 2: Momentum Forte
                if momentum_5m < MOMENTUM_SELL * 1.5:  # -0.045%
                    confirmations += 1
    ...
    def _calculate_atr_simple(self, rates) -> float:
        ...
        AJUSTADO: ATR mais baixo para operações mais ágeis
        """
        try:
            if len(rates) < 14:
                return 100.0  # AJUSTADO: 180 → 100 pontos
            ...
            # AJUSTADO: Mínimo 80 pontos para mais agilidade
            return max(atr, 80.0)
    ...
    def _open_position(self, signal: dict):
        ...
        try:
            ...
            if result and result.get('retcode') == 10009:  # TRADE_RETCODE_DONE
                ...
                try:
                    if not self.position_worker or not self.position_worker.is_running():
                        # Usar intervalo configurável (mínimo 0.5s)
                ...
                try:
                    ...
                    self.btc_logger.log_trade(trade_data)
                except Exception as e:
    ...
    def _update_trailing_from_worker(self, pos, current_bid, current_ask):
        ...
        try:
            ...
            if pos_type == 0:  # BUY
                ...
                profit_pontos = self.entry_price - current_price
    ...
    def _manage_trailing(self):
        """
        Gerencia trailing stop (FALLBACK quando worker não está ativo)
        """
    ...
    def _manage_position_trailing(self, pos):
        ...
        try:
            ...
            # Calcular lucro em PONTOS (não dólares!)
            if pos_type == 0:  # BUY
                profit_pontos = current_price - self.entry_price
            else:  # SELL
                profit_pontos = self.entry_price - current_price
            ...
            if self.trailing_active:
                # Calcular lucro protegido atual
                if pos_type == 0:  # BUY
                    lucro_protegido_pontos = self.trailing_stop_price - self.entry_price
                else:  # SELL
                    lucro_protegido_pontos = self.entry_price - self.trailing_stop_price
