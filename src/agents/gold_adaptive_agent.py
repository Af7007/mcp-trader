#!/usr/bin/env python3
"""
Gold Adaptive Agent - Gold Agent com auto-tuning e aprendizado automatico
"""
...
class GoldAdaptiveAgent(GoldLossZeroSimple):
    ...
    - Deteccao de regime de mercado a cada 1 hora
    ...
    def __init__(self, *args, auto_tuning_enabled: bool = True,
        ...
        """
        Inicializa Gold Adaptive Agent
        
        Args:
            auto_tuning_enabled: Habilitar otimizacao automatica
            optimization_interval: Otimizar a cada N trades (default: 50)
            min_trades_for_optimization: Minimo de trades para primeira otimizacao
        ...
        self.regime_check_interval = 3600  # 1 hora
        ...
        print(f"\n{'='*60}")
        print(f"GOLD ADAPTIVE AGENT - AUTO-LEARNING v2.0")
        print(f"{'='*60}")
        print(f"  Base: Gold Loss Zero Simple")
        print(f"  Modo: {'AGRESSIVO (Quick TP $1, Target $5+)' if aggressive_profit_mode else 'CONSERVADOR (TP $4-6)'}")
        print(f"  Auto-tuning: {'HABILITADO' if auto_tuning_enabled else 'DESABILITADO'}")
        print(f"  Otimizacao a cada: {optimization_interval} trades")
        print(f"  Minimo de trades: {min_trades_for_optimization}")
        print(f"  Deteccao de regime: A cada {int(self.regime_check_interval/60)} minutos")
        print(f"{'='*60}\n")
    ...
    def run(self):
        """
        Executa agente com auto-tuning
        """
        print(f"{'='*60}")
        print(f"INICIANDO AGENTE ADAPTATIVO")
        print(f"{'='*60}")
        ...
        try:
            ...
            while True:
                ...
                if self.auto_tuning_enabled and cycle % 20 == 0:
                ...
                # DEBUG: Verificar se sinais estao sendo gerados
                if cycle % 50 == 0:
            ...
            print(f"\n{'='*60}")
            print(f"AGENTE ADAPTATIVO PARADO PELO USUARIO")
            print(f"{'='*60}")
    ...
    def _check_market_regime(self):
        ...
        try:
            print(f"\n{'='*60}")
            print(f"[REGIME DETECTION] Analisando mercado...")
            print(f"{'='*60}")
    ...
    def _adapt_to_regime(self, regime: Dict):
        ...
        if regime_type == 'TRENDING_UP' or regime_type == 'TRENDING_DOWN':
            # Em trending: stops mais largos, trailing mais apertado
            adjustments['stop_loss_atr_multiplier'] = min(self.sl_atr_mult * 1.1, 8.0)
            adjustments['trailing_distance_mult'] = max(self.trailing_distance_mult * 0.9, 0.2)
            print(f"    Estrategia: SL mais largo, trailing mais apertado")
            
        elif regime_type == 'RANGING':
            # Em ranging: stops mais apertados, trailing ativa mais cedo
            adjustments['stop_loss_atr_multiplier'] = max(self.sl_atr_mult * 0.9, 3.0)
            adjustments['trailing_activation_mult'] = max(self.trailing_activation_mult * 0.8, 0.1)
            print(f"    Estrategia: SL mais apertado, trailing mais agressivo")
            
        elif regime_type == 'HIGH_VOLATILITY':
            # Alta volatilidade: stops mais largos
            adjustments['stop_loss_atr_multiplier'] = min(self.sl_atr_mult * 1.2, 8.0)
            print(f"    Estrategia: SL mais largo para volatilidade")
    ...
    def _run_auto_optimization(self):
        """
        Executa otimizacao automatica de parametros
        """
        print(f"\n{'='*60}")
        print(f"[AUTO-OPTIMIZATION #{self.optimization_count + 1}]")
        print(f"{'='*60}")
        ...
        try:
            ...
            # Sugerir ajustes (metodo conservador)
    ...
    def _validate_adjustments(self, current: Dict, suggested: Dict) -> bool:
        ...
        Maximo 20% de variacao por parametro + LIMITES DE SEGURANÇA para trailing distance
        """
        # NOVOS LIMITES DE SEGURANÇA para evitar problemas de lucro protegido
        SAFE_LIMITS = {
            'trailing_distance_atr_multiplier': {
                'min': 0.05,  # 5% do ATR mínimo (~$0.24)
                'max': 0.50,  # 50% do ATR máximo (~$2.40)
                'max_distance_pts': 200  # Máximo 200 pontos de trailing distance
            },
            'trailing_activation_atr_multiplier': {
                'min': 0.10,  # Ativar com pelo menos 10% do ATR
                'max': 1.00   # Ativar com máximo 100% do ATR
            },
            'stop_loss_atr_multiplier': {
                'min': 3.0,   # SL mínimo de 3x ATR (~$1.50)
                'max': 10.0   # SL máximo de 10x ATR (~$5.00)
            }
        }
        ...
        for param, new_value in suggested.items():
            ...
            # Validação original: Máximo 20% de variacao por vez
    ...
    def _print_final_report(self):
        ...
        """
        print(f"\n{'='*60}")
        print(f"RELATORIO FINAL - GOLD ADAPTIVE AGENT")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Gold Adaptive Agent com Auto-Tuning')
    ...
    parser.add_argument('--optimization-interval', type=int, default=50, help='Optimize every N trades')
