...
# -*- coding: utf-8 -*-
"""
Agente BTC Loss Zero v2.1.0 - VOLUME DINAMICO + SCALPING M5/M1
Estrategia: Trailing Stop ilimitado sem losses + Volume ajustavel por ATR
Baseado em gold_loss_zero_simple.py v2.0.0 com parametros adaptados para BTC

VERSAO 2.1.0 - VOLUME DINAMICO (SEGURANCA EM ATR ALTO):
- NOVO: Volume ajusta-se automaticamente ao ATR
  - ATR < $5: Volume 0.30 (CHEIO - mercado calmo)
  - ATR $5-10: Volume 0.20 (REDUZIDO - volatilidade media)
  - ATR > $10: Volume 0.10 (MINIMO - alta volatilidade)
- Vantagem: Risco sempre controlado independente do ATR
- SL permanece fixo $5.00 (mais simples de gerenciar)
...
- Score minimo M5: 3.5 (permite mais trades que v2.0.0)
- Trailing Stop: ativa com $2.00 de lucro (ajustado para scalping)
- Filtro ATR M5: max $2.00 (evita alta volatilidade)
- Filtro Spread: max $20.00 (BTC tem spreads maiores)
- Confirmacao M1: 2 velas consecutivas na direcao M5
- SL fixo: $5.00 (evita stops por ruido)

PARAMETROS BTC (mesma expectativa em $ que Gold):
- Volume: 0.30 lotes (vs Gold 0.03)
- Point: $0.01 (vs Gold $0.001)
- Point Value: $0.003/ponto (IGUAL ao Gold)
- SL/TP em dolares: IGUAIS ao Gold
...
class BTCLossZeroV2:
    """
    Agente BTC v2.0.0 com estrategia Loss 0 otimizado para BTCUSDc.

    Estrategia:
    - Trailing stop ilimitado
    - Parametros otimizados para Bitcoin (mesma expectativa $ que Gold)
    ...
    def __init__(
        self,
        symbol: str = "BTCUSDc",
        volume: float = 0.30,  # BTC: 0.30 lotes (equivalente a Gold 0.03 em point value)
        check_interval: int = 1,  # 1s (mais responsivo com FALLBACK)
        stop_loss_atr_multiplier: float = 5.0,  # Deprecated, usar fixed_sl_dollars
        fixed_sl_dollars: float = 5.0,  # SL fixo em dolares ($5.00 - mesma expectativa que Gold)
        use_buy: bool = True,
        use_sell: bool = True
    ):
        """
        Inicializa agente BTC Loss Zero v2.0.0 - ESTRATEGIA TRAILING STOP

        Args:
            volume: FIXADO em 0.30 lotes BTC (equivalente a Gold 0.03)
            fixed_sl_dollars: SL fixo em $5.00 (mesma expectativa $ que Gold)
            stop_loss_atr_multiplier: Deprecated (usar fixed_sl_dollars)

        PARAMETROS BTC vs GOLD:
            BTC Point Value = Gold Point Value = $0.003/ponto
            BTC Volume (0.30) = Gold Volume (0.03) × 10
            Porque BTC tick value ($0.01) = Gold tick value ($0.10) / 10

        ESTRATEGIA:
            1. Abre posicao com SL fixo $5.00
            2. SEM TP fixo (lucro ilimitado!)
            3. Quando lucro >= $2.00, trailing ATIVA
            4. Trailing protege lucro com distancia $1.00
            5. Trailing sobe/desce com o preco
            6. Fecha quando trailing e atingido (SEMPRE com lucro!)
        """
        self.symbol = symbol
        # Volume ajustado para BTC (0.30 = equivalente a Gold 0.03)
        self.volume = volume
        if volume < 0.01:
            print(f"AVISO: Volume muito baixo {volume}, minimo e 0.01")
            self.volume = 0.01
        elif volume > 1.0:
            print(f"AVISO: Volume {volume} lotes e muito alto! Certifique-se de ter margem suficiente.")
        self.check_interval = check_interval
        self.sl_atr_mult = stop_loss_atr_multiplier
        self.fixed_sl_dollars = fixed_sl_dollars  # SL fixo em dolares
        ...
        self.min_score_m5 = 3.5         # Score minimo M5 para entrada (reduzido de 4.0 para permitir mais trades)
        self.max_atr_m5_dollars = 20.0  # ATR M5 maximo em dolares (BTC mais volatil que Gold)
        self.max_spread_dollars = 20.0  # Spread maximo permitido (BTC pode ter spreads altos $15-20)

        # VOLUME DINAMICO v2.1.0 - Ajusta volume baseado em ATR (seguranca em alta volatilidade)
        self.base_volume = volume       # Volume base (0.30 para ATR baixo)
        self.use_dynamic_volume = True  # Ativar volume dinamico
        self.current_volume = volume    # Volume atual (sera ajustado por ATR)
        ...
        # Valor do ponto para BTC (sera calculado dinamicamente)
        self.point_value = None
        self.symbol_point = None  # Variacao de preco por ponto (ex: 0.01 para BTC)
        ...
        print(f"Agente BTC Loss Zero v2.1.0 - VOLUME DINAMICO + SCALPING M5/M1")
        print(f"   Symbol: {self.symbol}")
        print(f"   Volume Base: {self.base_volume} lotes (ajustavel por ATR)")
        print(f"   Volume Dinamico: ATIVO (0.10-0.30 conforme volatilidade)")
        print(f"   SL: ${self.fixed_sl_dollars:.2f} FIXO (mesma expectativa $ que Gold)")
        ...
        print(f"   Filtros: M5 (score >= 3.5) + ATR + Spread + M1 Timing")
        print(f"   Logger: ATIVO")
        print(f"")
        print(f"   TRAILING STOP OTIMIZADO PARA SCALPING:")
        ...
        print(f"   - Sobe ${self.protection_increment_dollar:.2f} a cada ${self.profit_step_for_increment_dollar:.2f} adicional")
        print(f"   - Exemplo: $3.00 lucro = protege $2.00 (1.00 + 1.00)")
        print(f"")

        print(f"   ESTRATEGIA v2.0.0:")
        print(f"   1. M5: Sinal PRINCIPAL (score >= 3.5)")
        print(f"   2. Filtro ATR: Evita alta volatilidade (max $20.00)")
        print(f"   3. Filtro Spread: Evita custos altos (max $20.00)")
        print(f"   4. M1: Timing de entrada (2 velas confirmacao)")
        print(f"   5. SL fixo ${self.fixed_sl_dollars:.2f} em dolares")
        print(f"   6. Trailing rapido para scalping")
        print(f"   7. Zero losses garantidos")

    def _calculate_point_value(self):
        """
        Calcula o valor de 1 ponto para BTC BTCUSDc.

        Para BTC:
        - Contract size: 0.01 BTC (normalmente)
        - Point: 0.01 (para cents account)
        - 1 ponto de movimento com 1 lote = $0.01
        - 1 ponto de movimento com 0.30 lote = $0.003 (equivalente a Gold 0.03)
        """
        try:
            symbol_info = self.mt5.get_symbol_info(self.symbol)

            if symbol_info:
                # Obter point (variacao de preco por ponto)
                self.symbol_point = symbol_info.get('point', 0.01)  # BTC: 0.01

                # Para BTC BTCUSDc:
                # Usar trade_tick_value que ja vem calculado pelo MT5
                ...
                if tick_value and tick_value > 0:
                    ...
                    print(f"[BTC] Point (variacao preco): {self.symbol_point}")
                    print(f"[BTC] Valor do ponto (tick_value): ${self.point_value:.4f} por lote")
                    print(f"[BTC] Com volume {self.volume}: 1 ponto = ${self.point_value * self.volume:.4f}")
                else:
                    # Fallback: calcular manualmente
                    contract_size = symbol_info.get('trade_contract_size', 0.01)
                    self.point_value = contract_size * self.symbol_point
                    print(f"[BTC] Point (variacao preco): {self.symbol_point}")
                    print(f"[BTC] Valor do ponto (calculado): ${self.point_value:.4f} por lote")
                    print(f"[BTC] Com volume {self.volume}: 1 ponto = ${self.point_value * self.volume:.4f}")
            else:
                # Fallback para BTC padrao
                self.symbol_point = 0.01
                self.point_value = 0.01  # $0.01 por ponto para 1 lote (conta cents)
                print(f"[BTC] Usando valor padrao - Point: {self.symbol_point}, Value: ${self.point_value:.2f} por lote")

        except Exception as e:
            print(f"Erro ao calcular point value: {e}")
            self.symbol_point = 0.01
            self.point_value = 0.01  # Fallback
    ...
    def _calculate_dynamic_volume(self, atr_dollars: float) -> float:
        """
        Calcula volume dinamico baseado em ATR para seguranca em alta volatilidade

        LOGICA v2.1.0:
        - ATR baixo (< $5):   Volume CHEIO (0.30) - Mercado calmo, risco baixo
        - ATR medio ($5-10):  Volume REDUZIDO (0.20) - Volatilidade media, risco medio
        - ATR alto (> $10):   Volume MINIMO (0.10) - Alta volatilidade, risco alto

        Vantagem: Risco TOTAL sempre controlado mesmo em ATR alto
        """
        if not self.use_dynamic_volume:
            return self.base_volume

        if atr_dollars < 5.0:
            # ATR baixo = mercado calmo = volume cheio
            volume = self.base_volume  # 0.30
            level = "CHEIO"
        elif atr_dollars < 10.0:
            # ATR medio = volatilidade moderada = reduzir volume
            volume = self.base_volume * 0.67  # 0.20
            level = "REDUZIDO"
        else:
            # ATR alto = alta volatilidade = volume minimo
            volume = self.base_volume * 0.33  # 0.10
            level = "MINIMO"

        # Arredondar para step do broker (0.01)
        volume = round(volume / 0.01) * 0.01

        # Garantir minimo do broker
        volume = max(0.01, volume)

        print(f"   [VOLUME DINAMICO] ATR ${atr_dollars:.2f} → Volume {volume:.2f} ({level})")
    ...
    def _get_simple_signal(self) -> dict:
        ...
        - PASSO 1: Analisar M5 para sinal PRINCIPAL (score >= 3.5)
        ...
        try:
            ...
            # v2.1.0: CALCULAR VOLUME DINAMICO baseado em ATR
            ...
            if atr_dollars_adjusted > self.max_atr_m5_dollars:
                print(f"   [FILTRO ATR] M5 ATR muito alto (${atr_dollars_adjusted:.2f} > ${self.max_atr_m5_dollars}), aguardando")
                return None
    ...
    def _analyze_m5_trend(self, rates_m5) -> dict:
        """
        Analisa M5 para sinal PRINCIPAL com score minimo 3.5
    ...
    def _calculate_atr_simple(self, rates) -> float:
        """
        Calcula ATR simples para SL/TP dinamico
        BTC: ATR otimizado para volatilidade do Bitcoin
        Retorna ATR em PONTOS MT5, nao em preco
        """
        try:
            if len(rates) < 14:
                # BTC: ATR padrao de ~$6.67 em pontos MT5
                # $6.67 / 0.01 (symbol_point) = 667 pontos
                # Equivalente ao Gold ($2.00 ATR max)
                return 667.0  # BTC: ATR padrao 667 pontos
            ...
            # BTC: Minimo 667 pontos (~$6.67 em preco, equivalente a $2.00 em expectativa)
            return max(atr_pontos, 667.0)

        except Exception as e:
            print(f"   Erro ao calcular ATR: {e}")
            return 667.0  # BTC: ATR padrao 667 pontos (fallback seguro)
    ...
    def _open_position(self, signal: dict):
        ...
        try:
            ...
            # IMPORTANTE: Usar point_value para cálculo correto (igual ao trailing)
            # Formula: pontos = loss_usd / (point_value × volume)
            # Depois: distance = pontos × symbol_point
            sl_dinheiro = self.fixed_sl_dollars

            # Converter perda em USD para distância de preço (usa MESMA fórmula do trailing)
            # v2.1.0: Usar current_volume (dinamico) ao inves de volume (fixo)
            # Exemplo: $5.00 / (point_value × 0.30) = pontos, depois × 0.01 = price distance
            volume_to_use = self.current_volume if hasattr(self, 'current_volume') and self.current_volume > 0 else self.volume
            pontos_para_sl = sl_dinheiro / (self.point_value * volume_to_use)
            sl_price_distance = pontos_para_sl * self.symbol_point

            # DEBUG: Mostrar calculos
            print(f"\n[DEBUG SL CALCULATION] - CRITICAL CHECK")
            print(f"  fixed_sl_dollars (target loss): ${self.fixed_sl_dollars:.2f}")
            print(f"  volume: {volume_to_use} (dinamico: {hasattr(self, 'current_volume')})")
            print(f"  point_value: ${self.point_value:.4f}")
            print(f"  pontos_para_sl: {pontos_para_sl:.1f}")
            print(f"  sl_price_distance (calculated): ${sl_price_distance:.3f}")
            print(f"  expected_loss_usd: ${pontos_para_sl * self.point_value * volume_to_use:.2f}")
    ...
    def _update_trailing_from_worker(self, pos, current_bid, current_ask):
        ...
        try:
            ...
            if trailing_active:
                ...
                # Exemplo (com novos valores):
                #   $1.50-3.00 lucro: protege $1.00
                #   $3.00-4.50 lucro: protege $2.00 ($1.00 + 1 nível)
                #   $4.50-6.00 lucro: protege $3.00 ($1.00 + 2 níveis)
                ...
                trailing_distance_dinheiro = min(trailing_distance_dinheiro, profit_dinheiro - 0.05) # Increased buffer
