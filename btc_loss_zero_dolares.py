#!/usr/bin/env python3
"""
BTC Loss Zero - VALORES EM DÓLARES FIXOS
Configuração:
- SL: $9.00
- Trailing ativa: $1.00
- Trailing sobe: $0.50
...
def calculate_volume_for_sl(sl_dollars: float, sl_points: int, point_value_per_lot: float) -> float:
    ...
    sl_points: Distância em pontos (ex: 120)
    point_value_per_lot: Valor de 1 ponto para 1 lote (ex: 0.01 para BTC)
...
# BTC SPECS (do MT5)
BTC_POINT_VALUE = 0.01               # $0.01 por lote por ponto
BTC_POINT_SIZE = 0.01                # Tick size

# CALCULAR PONTOS E VOLUME
# Usando ATR médio de 100 pontos para BTC
ATR_MEDIO = 100

# SL em pontos (1.2 × ATR = 120 pontos)
SL_POINTS = int(ATR_MEDIO * 1.2)  # 120 pontos

# Volume necessário para SL de $9.00 com 120 pontos
# $9.00 = volume × 120 × $0.01
# volume = $9.00 / (120 × $0.01) = $9.00 / $1.20 = 7.5 lotes
VOLUME = calculate_volume_for_sl(SL_DOLLARS, SL_POINTS, BTC_POINT_VALUE)
print(f"\n{'='*60}")
print(f"BTC LOSS ZERO - CONFIGURAÇÃO EM DÓLARES")
print(f"{'='*60}\n")
print(f"SL Desejado: ${SL_DOLLARS:.2f}")
print(f"SL Pontos: {SL_POINTS} pts (1.2 × ATR {ATR_MEDIO})")
print(f"Volume Calculado: {VOLUME:.2f} lotes")
print(f"")
print(f"Verificação:")
print(f"  {SL_POINTS} pts × {VOLUME:.2f} lotes × ${BTC_POINT_VALUE:.2f}/pt = ${SL_POINTS * VOLUME * BTC_POINT_VALUE:.2f}")
print(f"")

# Calcular multiplicadores baseados em $ ao invés de ATR
# Trailing ativa: $1.00 / ($0.01 × volume) = pontos
trailing_activation_points = TRAILING_ACTIVATION_DOLLARS / (BTC_POINT_VALUE * VOLUME)
trailing_activation_mult = trailing_activation_points / ATR_MEDIO

# Trailing distance: $0.50 / ($0.01 × volume) = pontos
trailing_distance_points = TRAILING_DISTANCE_DOLLARS / (BTC_POINT_VALUE * VOLUME)
...
agent = BTCLossZeroSimple(
    ...
    stop_loss_atr_multiplier=1.2,  # 120 pontos com ATR 100
