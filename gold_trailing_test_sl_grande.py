#!/usr/bin/env python3
"""
AGENTE GOLD TRAILING STOP - TESTE COM SL MAIOR
Versão com SL muito maior para evitar "Invalid stops"
...
class GoldTrailingTestFixed(GoldLossZeroSimple):
    """
    Agente de teste para trailing stop com SL maior
    SEM indicadores - abre posição IMEDIATAMENTE
    """
    
    def __init__(self, *args, **kwargs):
        """
        Configurações de teste com SL muito maior
        """
        ...
        print("="*80)
        print("GOLD TRAILING STOP - TESTE COM SL MAIOR")
        print("="*80)
        print("OBJETIVO: Testar trailing stop SEM indicadores")
        print("COMPORTAMENTO: Abre posição IMEDIATAMENTE")
        print("PROBLEMA RESOLVIDO: SL muito maior para evitar 'Invalid stops'")
        print("")
        print("CONFIGURAÇÕES ANTI-ERRO:")
        ...
        print(f"[TESTE] Objetivo: Evitar erro 'Invalid stops'")
        print("="*80)
        print("")
    
    def _open_position(self, signal: dict):
        """
        Override com SL MUITO maior para evitar "Invalid stops"
        """
        try:
            ...
            if signal["type"] == "BUY":
                ...
                # SL muito maior para BUY (abaixo do preço)
                sl_price = market_price - (sl_dinheiro / 2.0)  # Metade do valor em $ transformado em preço
                ...
                result = self.mt5.buy_market(
                    ...
                    sl=sl_price,
                    tp=0,  # SEM TP!
                    comment="Teste_Trailing_SL_Grande"
                )
            else:  # SELL
                market_price = tick['bid']  # Preco de venda
                
                # SL muito maior para SELL (acima do preço)
                sl_price = market_price + (sl_dinheiro / 2.0)  # Metade do valor em $ transformado em preço
            ...
            if result and result.get('retcode') == 10009:  # TRADE_RETCODE_DONE
                ...
                print(f"")
                print(f"{'='*60}")
                print(f"[POSICAO ABERTA PARA TESTE]: {signal['type']} ${market_price:.2f}")
                ...
                try:
                    trade_data = {
                        ...
                        'volume': self.volume,
                        'symbol': self.symbol,
                        'agent_version': "Teste_v1.0",
                        'reason': signal["reason"],
                        'status': "OPEN",
                        'comment': f"Teste_SL_Grande_{signal['type']}_{signal['reason']}"
                    }
                ...
                try:
                    trade_data_failed = {
                        ...
                        'reason': signal["reason"],
                        'status': "FAILED",
                        'comment': f"Teste_Failed_{signal['type']}_{signal['reason']}"
                    }
    ...
    def _analyze_and_open(self):
        ...
        print(f"[TESTE] ABRINDO POSIÇÃO DE TESTE COM SL GRANDE...")
    ...
    def _display_status(self, cycle: int):
        ...
        print(f"\n[TRAILING TEST SL GR] Ciclo #{cycle} | {self._get_time()}")
        print("="*50)
...
def main():
    print("TESTE TRAILING STOP COM SL GRANDE")
    print("OBJETIVO: Evitar erro 'Invalid stops'")
    print("COMPORTAMENTO: Abre posição imediata com SL muito maior")
    print("")
    
    # Criar agente de teste
    agent = GoldTrailingTestFixed(
        symbol='XAUUSDc',
    ...
    print("")
    print("O QUE ESPERAR:")
    print("1. Posição abre sem erro 'Invalid stops'")
    print("2. Trailing ativa em 30-60 segundos")
    print("3. Trailing se move com o preço")
    print("4. Teste completo do sistema")
    print("")
