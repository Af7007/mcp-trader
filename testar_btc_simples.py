...
# -*- coding: utf-8 -*-
"""
Teste simples do BTC Loss Zero sem logging complexo
Focado em corrigir os erros de indicadores
"""
...
class BTCLossZeroSimples:
    """
    Versão simplificada do agente BTC Loss Zero
    Foco em funcionamento básico sem erros
    """
    ...
    def __init__(
        ...
        stop_loss_dollars: float = 50.0,
        take_profit_dollars: float = 75.0,
        trailing_dollars: float = 25.0,
        use_buy: bool = True,
        use_sell: bool = True,
        bb_period: int = 20,
        ...
        """
        Inicializa agente simplificado
        """
        self.symbol = symbol
        self.volume = volume
        ...
        print(f"Agente BTC Loss Zero Simples inicializado!")
    ...
    def run(self):
        """
        Executa agente simplificado
        """
        print(f"AGENTE BTC LOSS ZERO SIMPLES")
        print(f"="*50)
        ...
        try:
            ...
            print(f"\nAgente BTC Loss Zero Simples parado pelo usuario")
    ...
    def _display_status(self, cycle: int):
        ...
        """
        print(f"\n{'='*50}")
        print(f"[AGENTE] BTC LOSS ZERO SIMPLES | Ciclo #{cycle} | {self._get_time()}")
        print(f"{'='*50}")
        
        # Status do agente
        print(f"[AGENTE]:")
        print(f"   Estado: LOSS ZERO SIMPLES")
        print(f"   Volume: {self.volume}")
        ...
        if self.base_agent.mt5:
            try:
                ...
                if tick:
                    ...
                    # Exibir indicadores (sem logging)
    ...
    def _display_indicators(self, current_price: float, cycle: int):
        """
        Exibe valores dos indicadores (sem logging)
        """
        try:
    ...
    def _get_signal(self) -> dict:
        ...
        try:
            ...
            if self.use_sell and current >= bb_upper and rsi > self.rsi_overbought and current > previous:
                return {
                    "type": "SELL",
                    "price": current,
            ...
            elif self.use_buy and current <= bb_lower and rsi < self.rsi_oversold and current < previous:
                return {
                    "type": "BUY",
                    "price": current,
        ...
        try:
            ...
            if signal["type"] == "BUY":
                ...
                if sl_price >= signal["price"] or tp_price <= signal["price"]:
                    print(f"❌ SL/TP inválidos para BUY")
                    print(f"   Preço: ${signal['price']:.2f}")
                ...
                result = self.base_agent.mt5.buy_market(
                    ...
                    sl=sl_price,
                    tp=tp_price,
                    comment=f"SIMPLES_BUY_{signal['strength']}"
                )
            else:  # SELL
                sl_price = signal["price"] + self.stop_loss_dollars
                tp_price = signal["price"] - self.take_profit_dollars
                ...
                # Validação
                if sl_price <= signal["price"] or tp_price >= signal["price"]:
                    print(f"❌ SL/TP inválidos para SELL")
                    print(f"   Preço: ${signal['price']:.2f}")
                    print(f"   SL: ${sl_price:.2f} (deve ser > preço)")
                    print(f"   TP: ${tp_price:.2f} (deve ser < preço)")
                    return
                ...
                result = self.base_agent.mt5.sell_market(
                    ...
                    sl=sl_price,
                    tp=tp_price,
                    comment=f"SIMPLES_SELL_{signal['strength']}"
                )
...
def main():
    ...
    Função principal
    """
    print("🧪 TESTE DO BTC LOSS ZERO SIMPLES")
    print("=" * 50)
    
    try:
        # Criar agente simplificado
        agent = BTCLossZeroSimples(
            symbol="BTCUSDc",
            volume=0.01,
            check_interval=30,
            stop_loss_dollars=50.0,
            take_profit_dollars=75.0,
            trailing_dollars=25.0,
            use_buy=True,
            use_sell=True,
            rsi_overbought=65.0,
            rsi_oversold=35.0
